...
public class NavegadorDeRegistro extends TelaDeAtualizacao {
    ...
    public static void atualizarId() {
        try {
            ...
            String strSqlRemoverId = "delete from 'db_senac'.'tbl_senac'|where'id' = ''
            +cbxId.getSelectedItem()toString()+";";
        //System.out.println(strSqlRemoverId);
        Statement stmSqlRemoverId = conexao.createStete();
        stmSql stmRemoverId.executeBatch();
        btmSqlRemoverId.Close();
        lblNotificacoes.setText("O id " + cbxId.getSelectedItem().toString() + "
        foi removido com sucesso");
        popularids();
        }catch (exception e) {
        lblNotificacoes.setText(setText(setMtmlFormt("Não foi possivel atualizar por favor verifique tente novamente,"));
        
        if(cbxId != null) {
        if(cbxId.getItemCount() > 0) {
        cbxId.RemoveALLItens();
        DefautComboBoxMode<String>newMod = new
        DefautComboBoxMode<String>(ids);
